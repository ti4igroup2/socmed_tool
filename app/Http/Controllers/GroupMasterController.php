...
class GroupMasterController extends Controller
{
    ...
    public function action(Request $r){
        ...
        if(isset($r->group_order)){
            ...
            $max = GroupMaster::selectRaw('max(group_order) as max')->where('group_type',$r->group_type)->first();
    ...
    public function updateOrder(Request $r){
        ...
        GroupMaster::where('id',$r->id)->update(["group_order"=>$r->order]);
        print json_encode("success");
    }
    ...
    public function count_group(Request $r){
        ...
        $count = GroupMaster::where('group_status',$operat,$r->status)->where('group_name','LIKE','%'.$r->search.'%')->count();
        json_true($count);
    }
    public function get_group(Request $r){
        ...
        if($r->group==""){$operat2 = "!="; }else{$operat2 = "=";}
        $sosmed = GroupMaster::where('group_status',$operat,$r->status)->where('group_name','LIKE','%'.$r->search.'%')
