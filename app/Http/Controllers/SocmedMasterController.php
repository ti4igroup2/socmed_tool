...
class SocmedMasterController extends Controller
{
    ...
    public function count_sosmed(Request $r){
        ...
        $count = SocmedMaster::where('socmed_status',$operat,$r->status)->where('group_id',$operat2,$r->group)->where(function($query) use ($r){
    ...
    public function get_sosmed(Request $r){
        ...
        $sosmed = SocmedMaster::where('socmed_status',$operat,$r->status)->where('group_id',$operat2,$r->group)->where(function($query) use ($r){
            $query->where('socmed_name','LIKE','%'.$r->search.'%')
            ->orwhere('socmed_type','LIKE','%'.$r->search.'%');
        })
        ->with('lastCounts','groupMaster')
        ->skip($r->offset)->limit($r->limit)->orderByRaw($r->orderByCol." ".$r->orderByAct)->get();
        // ->orderBy($r->orderByCol, $r->orderByAct)
        //  dump($sosmed);
        }else{
            $last = SocmedLastRetrieved::orderby($r->orderByCol, $r->orderByAct)->orderByRaw('date(created_at) desc')->distinct()->get(['socmed_id','socmed_total','created_at'])->pluck('socmed_id')->toArray();
            // dd($last);
            $order_array = implode(",",$last);
            $sosmed = SocmedMaster::whereIn('id',$last)->
            where('socmed_status',$operat,$r->status)->where('group_id',$operat2,$r->group)->where(function($query) use ($r){
                $query->where('socmed_name','LIKE','%'.$r->search.'%')
                ->orwhere('socmed_type','LIKE','%'.$r->search.'%');
            })
            ->with('lastCounts','groupMaster')
            ->skip($r->offset)->limit($r->limit)->orderByRaw('FIELD(id,'.$order_array.')')->get();
        }
